...
function numberOfCharacters(string) {
  ...
  for (var i = 0; i < string.length; i++) {
    if (objetoNuevo.hasOwnProperty(string[i])) {
      objetoNuevo[string[i]] = objetoNuevo[string[i]] +1;
    } else {
...
function capToFront(s) {
  ...
  for (let letter of s) {
    if (letter == letter.toUpperCase()) {
      sortedUpStr.push(letter);
    }
    else {
      sortedLowStr.push(letter);
    }
  }
  return sortedUpStr.join("") + sortedLowStr.join("");
}
...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
  let arrayReversed = [];
  for (let elemento of str.split(" ")) {
    arrayReversed.push(elemento.split("").reverse().join(""));
  }
...
function deleteAbc(cadena){
  ...
  for (elemento of cadena.split('')) {
    if (elemento === 'a' || elemento === 'b' || elemento === 'c') {
      {continue};
    }
    else { 
      cadenaModificada.push(elemento);
    }
  }
